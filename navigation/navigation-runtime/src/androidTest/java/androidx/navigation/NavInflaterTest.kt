...
class NavInflaterTest {
    ...
    @Test
    fun testActionGesture() {
        val context = ApplicationProvider.getApplicationContext() as Context
        val navInflater = NavInflater(context, TestNavigatorProvider())
        val graph = navInflater.inflate(R.navigation.nav_action_gesture)
        val startDestination = graph.findNode(graph.startDestination)
        val action = startDestination?.getAction(R.id.my_action)
        assertThat(action?.navGesture?.gesture)
                .isEqualTo(0)
        assertThat(action?.navGesture?.widgetId)
                .isEqualTo(R.id.start_test)
    }

    @Test
    fun testActionOnlyGesture() {
        val context = ApplicationProvider.getApplicationContext() as Context
        val navInflater = NavInflater(context, TestNavigatorProvider())
        val graph = navInflater.inflate(R.navigation.nav_action_gesture)
        val startDestination = graph.findNode(graph.startDestination)
        val action = startDestination?.getAction(R.id.my_action1)
        assertThat(action?.navGesture?.gesture)
                .isEqualTo(0)
        assertThat(action?.navGesture?.widgetId)
                .isEqualTo(R.id.start_test)
